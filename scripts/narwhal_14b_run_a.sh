...
#SBATCH --output=c2c-14b-A_%j.out
#SBATCH --error=c2c-14b-A_%j.err
#
# =============================================================================
# Issue #52: Run A — WITH speaker models (Qwen2.5-14B, 8K context, V100)
# =============================================================================
#
# This is ONE HALF of the A/B experiment. Run B (without speakers)
# should be submitted separately with identical settings EXCEPT
# CHAT_TO_COP_USE_SPEAKER_MODELS=false.
#
# 8-hour walltime to accommodate the observed ~28s/message rate on
# V100 with 14B at 8K context. Expected duration: ~7 hours.
#
# Results are archived to $ARCHIVE_HOME so they survive the 30-day
# $WORKDIR purge.
# =============================================================================
...
JOB_ID="${SLURM_JOB_ID:-$$}"
DB_PATH="$OUTPUT_DIR/replay_14b_with_speakers_${JOB_ID}.db"
LOG_FILE="$OUTPUT_DIR/replay_14b_runA_${JOB_ID}.log"
...
echo "=============================================================================="
echo "Chat-to-CoP: 14B Run A — WITH Speaker Models (#52)"
echo "=============================================================================="
echo "Job ID:     $JOB_ID"
echo "Node:       $(hostname)"
...
cleanup() {
    ...
    # Also archive the slurm output
    for f in "$WORK/chat-to-cop/c2c-14b-A_${JOB_ID}".{out,err}; do
...
# =============================================================================
# Run A: WITH speaker models
# =============================================================================

echo "=============================================================================="
echo "STARTING RUN A: WITH speaker models"
echo "Expected duration: ~7 hours at ~28s/message"
echo "=============================================================================="
...
echo "Output DB:   $DB_PATH"
echo ""
echo "Next step: Submit Run B with identical settings + CHAT_TO_COP_USE_SPEAKER_MODELS=false"
echo "Script:    scripts/narwhal_14b_run_b.sh"
echo "=============================================================================="
