...
from chat_to_cop.ingestion.multiline import reassemble_multiline
...
def _make_degrading_backend(
    ...
    if asksage_config:
        ...
        azure_endpoint = config.llm.llm_azure_endpoint or None
        primary = OpenAICompatibleBackend(
            ...
            azure_api_version=config.llm.llm_azure_api_version if azure_endpoint else None,
        )
...
async def run_replay(
    ...
    # Load supplemental glossary if configured. The file content is appended
    # to DEFAULT_GLOSSARY and passed through Supervisor -> ChannelAgent so
    # the LLM sees exercise-specific callsigns and track numbers.
    glossary: str | None = None
    if config.agent.glossary_file:
        gpath = config.agent.glossary_file
        if os.path.isfile(gpath):
            with open(gpath, encoding="utf-8") as gf:
                supplement = gf.read().strip()
            glossary = DEFAULT_GLOSSARY + "\n\n" + supplement
            logger.info(f"Loaded supplemental glossary from {gpath} ({len(supplement)} bytes)")
        else:
            logger.warning(f"Glossary file not found: {gpath}, using DEFAULT_GLOSSARY only")
    ...
    async with WorldStateStore(config.db_path) as store:
        if irc_url:
            ...
            irc_client = IRCClient(
                irc_url,
                channels,
                discover_channels=config.irc_discover_channels,
                max_channels=config.irc_max_channels,
            )
            message_source = reassemble_multiline(irc_client.iter_messages())
            logger.info(f"Live IRC: {irc_url}")
            logger.info(f"Channels: {', '.join(channels)}")
            if config.irc_discover_channels:
                logger.info(f"Channel discovery: ON (max {config.irc_max_channels})")
        else:
            assert path is not None
            message_source = reassemble_multiline(replay_messages(path, speed=speed))
...
def main() -> None:
    ...
    parser.add_argument(
        "--azure",
        action="store_true",
        help="Use Azure OpenAI (requires CHAT_TO_COP_LLM_AZURE_ENDPOINT or --azure-endpoint)",
    )
    parser.add_argument(
        "--azure-endpoint",
        default=None,
        help="Azure OpenAI endpoint (e.g. https://myresource.openai.azure.com)",
    )
    parser.add_argument(
        "--azure-api-version",
        default=None,
    ...
    # Azure mode: pass through to OpenAICompatibleBackend (same class, Azure client)
    if args.azure:
        azure_endpoint = args.azure_endpoint or config.llm.llm_azure_endpoint
        if not azure_endpoint:
            logger.error("--azure requires --azure-endpoint or CHAT_TO_COP_LLM_AZURE_ENDPOINT")
            sys.exit(1)
        config.llm.llm_azure_endpoint = azure_endpoint
        if args.azure_api_version:
            config.llm.llm_azure_api_version = args.azure_api_version
        if not args.model:
            config.llm.llm_model = "gpt-4o"
