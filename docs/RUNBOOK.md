...
Operational procedures for chat-to-cop demos and development.
...
**Key tie-in:** The very first message in the demo is the **HYDRO_SL SITREP** that appears verbatim on **Slide 5** of [BRIEFING_SLIDES.md](internal/BRIEFING_SLIDES.md). Show the slide first, then run the demo and let the audience watch the same message get extracted live. That's the moment the system "lands."

**For MASH itself** (not external presentations), see [GO_NOGO.md](GO_NOGO.md) -- the decision tree, pre-event checklist, recovery procedures, and hard rules for the actual event.

### Pick your backend

All options below run `scripts/quick_test.py` -- only the backend changes. Pick whichever you have credentials for.
...
```
Use this only if internet is flaky at the venue or your API keys are not at hand.

### Tonight (the night before)

1. **Decide which backend** based on what credentials you have ready.
2. **Run the demo end-to-end once** with that exact command. Make sure it works clean.
3. **Note the wall-clock time** so you know what to expect tomorrow.
4. **Have a second backend ready as backup** (e.g., Gemini primary, Anthropic backup).

Fix any permission/auth issues tonight -- not in front of the audience.
...
In a terminal already in the repo root, with your chosen backend command. Total runtime: 20-60s depending on backend.
...
If anyone asks *"how does it scale?"*, point at **Slide 9** (7 backends validated, sub-10s latency) and **Slide 9b** (every backend has 4-14x headroom over peak DASH 3 traffic).

If anyone asks *"can you show me the code?"*:
...
- `src/chat_to_cop/models/cop_update.py` -- the Pydantic schema

If anyone asks about the *research story* (RQ1/calibration):
- [SPEAKER_MODEL_RESULTS.md](research/SPEAKER_MODEL_RESULTS.md) -- the speaker model A/B finding
- [BENCHMARK_RESULTS.md](research/BENCHMARK_RESULTS.md) -- calibration profile, multi-backend comparison
...
MODEL=qwen3:30b-a3b N=50 sbatch scripts/blueback_replay_bench.sh  # with overrides
```
...
- none_false_positive_rate increases more than 5pp: the change over-classifies messages as noise
- prompt_approx_tokens grows past the CI ceiling (17,500 chars / ~4,375 tokens):
  `test_prompt_budget.py` will fail; raise PROMPT_CHAR_CEILING and explain the trade-off
